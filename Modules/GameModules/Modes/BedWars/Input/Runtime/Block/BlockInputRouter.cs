...
{
    public class BlockInputRouter : IGameLoop
    {
        private const float PlaceBlockSearchDistance = 100f;
        private const int RemoveBlockCount = 1;

        private readonly IInput _input;
        private readonly IBlockRules _rules;
        private readonly IDroppedItemCommunication _droppedItemsCommunication;
        ...
        private readonly ISoundSource _placeBlockSoundSource;
        ...
        private readonly bool _needDisableOnUi;
        ...
        public BlockInputRouter(IInput input, IBlockRules rules, BlocksCommunication blocksCommunication,
               ICameraMovement cameraMovement, IInventory inventory, IInventoryView inventoryView,
               ItemsList itemsList, LayerMask layerMask, OutlineBlockView outlineView,
               IDroppedItemCommunication droppedItemsCommunication, Transform blockPlacePoint,
               Transform cameraPivot, Hand hand, bool needDisableOnUi, GameObject dropButton, IBlockAnimation blockAnimation,
               ISoundSource placeBlockSoundSource, IBlockDamageSource blockDamageSource,
               ...
               IItemExperienceEventRule placeBlockEventRule, IItemExperienceEventRule destroyBlockEventRule)
        {
            _input = input;
            _rules = rules;
            _droppedItemsCommunication = droppedItemsCommunication;
            ...
            _hand = hand;
            _needDisableOnUi = needDisableOnUi;
            _placeBlockSoundSource = placeBlockSoundSource;
        ...
        {
            if (EventSystem.current.IsPointerOverGameObject() && _needDisableOnUi)
            {
            ...
            _dropButton?.SetActive(
                _itemsList.TryGetItemById(_inventory.ItemIdBy(_inventory.SelectedSlotIndex), out var targetItem) &&
                targetItem.CanDrop && _inventory.HasItemsWith(_inventory.ItemIdBy(_inventory.SelectedSlotIndex), 1));
            ...
            if (TryGetBlock(out IBlock blockItem, out Block blockTemplate))
            {
                ...
                {
                    if (TryPlaceBlock(blockItem, blockTemplate) || TryPlaceBlockNear(blockItem, blockTemplate))
                    {
            ...
            if (_hand.CurrentItem.TryConvertTo(out IPick pick))
            {
                if (_input.RemoveBlock())
                {
                    ...
                    if (TryRemoveBlock(out Vector3 removePosition, out string blockId, pick.Damage))
                    {
                        if (_itemsList.TryGetItemById(blockId, out IItem item))
                        {
                            ...
                            if (item.CanDrop)
                                DropItem(removePosition, blockId);
                        }
                    }
                }
            }
        ...
        private bool TryPlaceBlock(IBlock blockItem, Block blockTemplate)
        {
            if (PositionPlace(out var positionHit, out var positionNormal) == false)
                return false;

            if (_rules.CanPlace(positionHit, blockTemplate.Size))
                if (_blocksCommunication.TryPlace(blockTemplate, positionHit, blockItem.Id, blockItem.Health))
                    return true;

            if (_rules.CanPlace(positionHit + positionNormal, blockTemplate.Size))
                if (_blocksCommunication.TryPlace(blockTemplate, positionHit + positionNormal, blockItem.Id, blockItem.Health))
                    return true;

            return false;
        }

        private bool TryPlaceBlockNear(IBlock blockItem, Block blockTemplate)
        {
            for (var i = 1; i <= _rules.PlaceDistance; i++)
                if (_blocksCommunication.TryPlaceNear(blockTemplate, NearPositionPlace(i), blockItem.Id, blockItem.Health))
                    return true;
        ...
        {
            _inventory.TryRemove(_inventory.SelectedSlotIndex, RemoveBlockCount);
            _inventory.Visualize(_inventoryView);
        }

        private void DropItem(Vector3 position, string id)
        {
            if (_itemsList.TryGetItemById(id, out var item) == false)
                throw new InvalidOperationException();

            _droppedItemsCommunication.DropFromPosition(item, position, new Vector3(Random.Range(-0.3f, 0.3f), 1, Random.Range(-0.3f, 0.3f)));
        }
        ...
        private bool TryGetBlockItem(out IBlock block)
        {
            ...
            if (_inventory.CanRemove(_inventory.SelectedSlotIndex, RemoveBlockCount) == false)
                return false;
